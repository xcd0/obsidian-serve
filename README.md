...
**方法1: 手動で `index.md` を作成（推奨）**

Obsidianで `Public/index.md` を作成して、ホームページの内容を記述してください：
...
**方法2: ワークフローの自動生成機能を使用**

v0.0.2以降では、`index.md` が存在しない場合、ワークフローが自動的にデフォルトの `index.md` を生成します。以下の手順で最新のワークフローに更新してください：

1. プラグインを最新版に更新
2. Obsidian設定 → Community plugins → GitHub Pages Publish → 管理タブ
3. 「GitHub Actions をセットアップ」ボタンをクリック
4. Vaultをcommit & push

次回のビルドから、自動的に `index.md` が生成され、最近の10件のノートへのリンクが含まれます。
