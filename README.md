...
2. **ホームページ (index.md) の生成**
   - 「ホームページ」タブから `index.md` をワンクリックで生成
   - ページタイトル、説明文、最近のノート一覧をカスタマイズ可能
   - 生成された `index.md` は公開対象ディレクトリ直下に保存され、自由に編集できる
...
### サイトのホームページが表示されない / `index.xml`が表示される

**症状**: `https://<username>.github.io/<repository-name>/` にアクセスすると `index.xml`（RSSフィード）が表示される。

**原因**: `Public/` ディレクトリのルートに `index.md` が存在しないため、Quartzが `index.html`（ホームページ）を生成していません。

**解決策**:

**プラグインの「ホームページ」タブから生成（推奨）**

1. Obsidian設定 → Community plugins → GitHub Pages Publish
2. **「ホームページ」タブ**を開く
3. ページタイトル、説明文、最近のノート設定を入力
4. **「生成」ボタン**をクリック

プラグインが自動的に `Public/index.md` を生成します。設定から以下をカスタマイズできます：
- ページタイトル（デフォルト: "Home"）
- ウェルカムメッセージの見出し（デフォルト: "Welcome"）
- 説明文
- 最近のノートを含めるか
- 最近のノートの件数（1〜30件）

生成された `index.md` はVault内の `Public/index.md` に保存されるため、自由に編集できます。
